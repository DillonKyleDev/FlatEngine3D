...
{	
	...
	// Vulkan
	void UpdateVulkanMeshes()
	{
		F_VulkanManager->SetMeshes(GetLoadedScene()->GetMeshesByMaterial());

		for (std::pair<std::string, std::vector<Mesh>> meshPair : GetLoadedScene()->GetMeshesByMaterial())
		{
			F_VulkanManager->AddMeshesByMaterial(meshPair);
		}
	}
	...
	void HandleEvents(bool& quit)
	{
		...
		//glfwGetFramebufferSize(F_VulkanManager->GetWinSystem().GetWindow(), &fb_width, &fb_height);
		//if (fb_width > 0 && fb_height > 0 && (F_Window->m_SwapChainRebuild || F_Window->m_MainWindowData.Width != fb_width || F_Window->m_MainWindowData.Height != fb_height))
		//{
		//	ImGui_ImplVulkan_SetMinImageCount(F_Window->m_MinImageCount);
		//	ImGui_ImplVulkanH_CreateOrResizeWindow(F_Window->m_Instance, F_Window->m_PhysicalDevice, F_Window->m_Device, &F_Window->m_MainWindowData, F_Window->m_QueueFamily, F_Window->m_Allocator, fb_width, fb_height, F_Window->m_MinImageCount);
		//	F_Window->m_MainWindowData.FrameIndex = 0;
		//	F_Window->m_SwapChainRebuild = false;
		//}
		//if (glfwGetWindowAttrib(F_Window->GetWindow(), GLFW_ICONIFIED) != 0)
		//{
	...
	{	
		...
		//Create dockable background space for all viewports to attach to
	...
	{		
		...
		// Multi-Viewports (disabled for now)
